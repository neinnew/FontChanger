...
public class Mod : IUserMod
{
    ...
    public void OnSettingsUI(UIHelper helper)
    {
        ...
        UITextField? textfieldFontSize = null;
        textfieldFontSize = helper.AddTextfield("Font Size (default: 16)", FontChanger.FontSize.ToString(), _ => { }, val =>
        {
            try
            {
                HideInvalidFontSize();
                FontChanger.FontSize = Mathf.Clamp(int.Parse(val), 8, 30);
                ModConfig.Save();
            }
            catch
            {
        ...
        textfieldFontSize!.width = 50;

        void ShowInvalidFontSize()
        {
            textfieldFontSize!.color = Color.yellow;
        }

        void HideInvalidFontSize()
        {
            textfieldFontSize!.color = Color.white;
        }
